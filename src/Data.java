...
public class Data {
    ...
    public String[] getDataPart1(){
        return dataPart1;
    }

    public String[] addDataPart2(int nPeople, HashMap<String, HashMap<String, Integer>> people){
        String[] dataPart2 = new String[dataPart1.length + 2 * nPeople];

        for(int i = 0; i < dataPart1.length; i++){
            dataPart2[i] = dataPart1[i];
        }
        int oldDataSize = dataPart1.length;

        Iterator<String> iter = people.keySet().iterator();
        int counter = 0;
        while(iter.hasNext()){
            String name = iter.next();
            dataPart2[oldDataSize + 2 * counter] = name + " would gain 0 happiness units by sitting next to Myself";
            dataPart2[oldDataSize + 2 * counter + 1] = "Myself would gain 0 happiness units by sitting next to " + name;
            counter++;
        }
